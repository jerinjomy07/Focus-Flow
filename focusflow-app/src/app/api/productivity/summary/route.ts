// src/app/api/productivity/summary/route.ts
// FocusFlow — User Productivity Summary API Handler
// GET: Aggregates productivity data using 3 mutually exclusive modes:
// - Mode A: date=YYYY-MM-DD
// - Mode B: period=today|yesterday|week|month (defaults to today if no params)
...
export async function GET(req: Request) {
  try {
    ...
    const summary = await ProductivityService.getSummary(
      session.user.id,
      parsedQuery.data
    );
